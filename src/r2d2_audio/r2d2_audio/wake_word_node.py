...
Available built-in models: alexa, hey_mycroft, hey_jarvis, hey_rhasspy, timer, weather
...
# Suppress CUDA/GPU warnings - Pi4 has no GPU
warnings.filterwarnings('ignore', category=UserWarning, module='onnxruntime')
os.environ['ORT_LOGGING_LEVEL'] = '3'  # suppress onnxruntime warnings
...
SAMPLE_RATE  = 16000
CHANNELS     = 8
PROCESSED_CH = 0       # CH0 from ReSpeaker
CHUNK_SIZE   = 1280    # 80ms at 16kHz – required by openWakeWord

# Defaults
DEFAULT_THRESHOLD      = 0.5
DEFAULT_LISTEN_TIMEOUT = 5.0
DEFAULT_COOLDOWN       = 2.0  # seconds before re-triggering allowed
...
class WakeWordNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('cooldown', DEFAULT_COOLDOWN)
        ...
        self._running       = True
        self._last_detected = 0.0  # timestamp of last detection
        ...
        if model_path:
            ...
            jarvis = [p for p in available if 'hey_jarvis' in p]
            if not jarvis:
                self.get_logger().error(f'hey_jarvis not found. Available: {available}')
                return
            model_paths = [jarvis[0]]
            self.get_logger().warn(
                f'Using placeholder model: {os.path.basename(model_paths[0])}. '
                'Train custom "Hey R2D2": https://github.com/dscripka/openWakeWord'
            )

        # Load model with Silero VAD
        ...
        self.get_logger().info(
            f'Model loaded | threshold={self.threshold} | '
            f'cooldown={self.cooldown}s | listen_timeout={self.listen_timeout}s'
        )
        ...
        self.get_logger().info('Wake word node ready – say "Hey Jarvis" to test')
    ...
    def _capture_audio(self):
        ...
        self.get_logger().info(f'Audio capture started (device {device_idx})')

        def callback(indata, frames, time_info, status):
            if status:
                self.get_logger().warn(f'Audio: {status}', throttle_duration_sec=5.0)
            try:
                self.audio_queue.put_nowait(indata[:, PROCESSED_CH].copy())
            except queue.Full:
                pass
