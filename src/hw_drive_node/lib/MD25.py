...
class MD25 :
  ...
  # Constructor
  def __init__(self, address=0x58, mode=1, debug=True):
  ...
  def resetEncoders(self):
    ...
    print('Encoder Registers reset!')
    
  def showSoftwareVersion(self):
    #"Reads and prints the Softwar Revision of the MD25"
    self._software_revision = self.i2c.readS8(self.__MD25_SOFTWARE_VERSION)
    print('MD25 SW Version: " + str((self._software_revision))')
    

  def showBatteryVoltage(self):
    #"Reads the battery voltage"
    self._battery_voltage = self.i2c.readS8(self.__MD25_BATTERY_VOLTAGE)
    self._battery_voltage = self._battery_voltage / 10.0
    print('DBG: BATTERY_VOLTAGE = %(batt_value).1f%(unit)s' % {'batt_value' : self._battery_voltage, 'unit' : 'V'})

  def showMotorCurrent(self):
    #"Reads the motor current for both motors"
    self._motor_1_current = self.i2c.readS8(self.__MD25_MOTOR_1_CURRENT)
    self._motor_2_current = self.i2c.readS8(self.__MD25_MOTOR_2_CURRENT)
    print('DBG: MOTOR_CURRENT: Motor 1: ' + str((self._motor_1_current)) + ' Motor 2: ' + str((self._motor_2_current)))
    
  def readData(self):
    #"Reads the data from the MD25" 
    self._enc_1a = self.i2c.readU8(self.__MD25_ENC_1A) <<24
    self._enc_1b = self.i2c.readU8(self.__MD25_ENC_1B) <<16
    self._enc_1c = self.i2c.readU8(self.__MD25_ENC_1C) <<8
    self._enc_1d = self.i2c.readU8(self.__MD25_ENC_1D)
    self._enc_1 = self._enc_1a + self._enc_1b + self._enc_1c + self._enc_1d
    self._enc_2a = self.i2c.readS8(self.__MD25_ENC_2A)
    self._enc_2b = self.i2c.readS8(self.__MD25_ENC_2B)
    self._enc_2c = self.i2c.readS8(self.__MD25_ENC_2C)
    self._enc_2d = self.i2c.readS8(self.__MD25_ENC_2D)
  ...
  def showData(self):
      ...
      print('DBG: ENC_1 = %6d' % self._enc_1)
      print('reg 1a: ' + str((self._enc_1a)))
      print('reg 1b: ' + str((self._enc_1b)))
      print('reg 1c: ' + str((self._enc_1c)))
      print('reg 1d: ' + str((self._enc_1d)))
            
      print('Raw Data Motor 2: ' + str((self._enc_2a)))
